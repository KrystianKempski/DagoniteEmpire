...
namespace DA_Business.Repository.CharacterReps
{
    public class TraitEquipmentRepository : ITraitRepository<TraitEquipmentDTO>
    {
        ...
        public async Task<TraitEquipmentDTO> Add(TraitEquipmentDTO objDTO, int Id)
        {

            using var contex = await _db.CreateDbContextAsync();
            var equipment = await contex.Equipment.Include(u => u.Traits).FirstOrDefaultAsync(c => c.Id == Id);
            if (equipment == null)
                return null;
            var obj = _mapper.Map<TraitEquipmentDTO, TraitEquipment>(objDTO);
            equipment.Traits.Add(obj);
            await contex.SaveChangesAsync();
        ...
        public async Task<IEnumerable<TraitEquipmentDTO>> GetAll(int? charId =null)
        {
            ...
            if (charId == null || charId < 1)
                return _mapper.Map<IEnumerable<TraitEquipment>, IEnumerable<TraitEquipmentDTO>>(await contex.TraitsEquipment.AsNoTracking().Include(u => u.Bonuses).AsSplitQuery().ToListAsync());
           return _mapper.Map<IEnumerable<TraitEquipment>, IEnumerable<TraitEquipmentDTO>>(await contex.TraitsEquipment.AsNoTracking().Include(u => u.Bonuses).Where(u => u.Equipment.FirstOrDefault(c=>c.Id == charId)!=null).AsSplitQuery().ToListAsync());
        }
        ...
        public async Task<IEnumerable<TraitEquipmentDTO>> GetAllApproved(bool addUnique = false)
        {
            using var contex = await _db.CreateDbContextAsync();
            if (addUnique)
                return _mapper.Map<IEnumerable<TraitEquipment>, IEnumerable<TraitEquipmentDTO>>(await contex.TraitsEquipment.AsNoTracking().Include(u => u.Bonuses).Where(t => t.TraitApproved == true).AsSplitQuery().ToListAsync());
                    
            return _mapper.Map<IEnumerable<TraitEquipment>, IEnumerable<TraitEquipmentDTO>>(await contex.TraitsEquipment.AsNoTracking().Include(u => u.Bonuses).Where(t=>t.TraitApproved==true && t.IsUnique == false).AsSplitQuery().ToListAsync());
        }

        public async Task<TraitEquipmentDTO> GetById(int id)
        {
            using var contex = await _db.CreateDbContextAsync();
            var obj = await contex.TraitsEquipment.AsNoTracking().Include(u=>u.Bonuses).AsSplitQuery().FirstOrDefaultAsync(u => u.Id == id);
            if (obj != null)
            {
                return _mapper.Map<TraitEquipment, TraitEquipmentDTO>(obj);
            }
